...
Private Sub spnId_SpinUp()
    '商品IDをスピンアップしたとき
    
    'IsNumeric()で例外処理
    If Not IsNumeric(txtProdId.Value) Then
        txtProdId.Value = 0
    End If
    
    txtProdId.Value = txtProdId.Value + 1


End Sub

Private Sub spnId_SpinDown()
    '商品IDをスピンダウンしたとき

    'IsNumeric()で例外処理（数値以外が入っていた場合は 0 を入れる）
    If Not IsNumeric(txtProdId.Value) Then
        txtProdId.Value = 0
    End If
...
Private Sub txtProdId_Change()
    ...
    '商品IDを入力したら、自動的にそれに紐づいた単価がテキストボックスに挿入されるようにする
    ...
    On Error GoTo vlookError
        ...
        '単価
        txtPrice.Value = _
        WorksheetFunction.VLookup(CInt(txtProdId.Value), Worksheets("リスト").Range("C3:E" & ProdMaxRow), 3, False)
...
Private Sub spnNum_SpinUp()
    '数量のスピンアップ
    If Not IsNumeric(txtNum.Value) Then
        txtNum.Value = 0
    End If
    
    txtNum.Value = txtNum.Value + 1
